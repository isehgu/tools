...
class IssueTrackerModelDefault extends IssueTrackerModel
{
	...
	public function getIssuesBySrting($string, $project, $offset)
	{
		...
		$conds = "`project_name` = '".$project."' 
		          AND `deleted` = 0 
		          AND (`issue_id` LIKE '%".$string."%' OR `title` LIKE '%".$string."%' OR `incident` LIKE '%".$string."%' OR `solution` LIKE '%".$string."%')";
	...
	public function addIssue($postData, $userId, $userName)
	{
		$data = array(
			...
			'incident'       => $postData['bt_incident'], 
			'solution'       => $postData['bt_solution'],
