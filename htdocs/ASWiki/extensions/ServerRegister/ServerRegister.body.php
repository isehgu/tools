<?php
/** @see ServerRegisterModelDefault **/
require_once dirname(__FILE__) . '/Models/ServerRegisterModelDefault.php';
 ...
 *
 */
class ServerRegister extends SpecialPage
{
	/**
	...
	{
		parent::__construct('ServerRegister');
	...
	protected function _processActionRequest(array $namespace, $isParserHook = false, $args = array())
	{
		...
		if (array_key_exists($action, $this->_config->getPermissions())) {
				...
				if ($controller->hasPermission($action)) {
					...
					$controller->setModel(new ServerRegisterModelDefault());
