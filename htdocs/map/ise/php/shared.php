...
///////////////////////////////////////////////////////////////////////////////////////////
    //Return a list of the latest 20 releases on this template in event history table
    ...
    function f_getReleaseIdListfromTemplateId($template_id)
    {
        global $db;
        $list = array();
        $sql_query = "select distinct release_id from event_history where template_id = $template_id and release_id != 0 order by release_id desc limit 20";
        $result = $db->query($sql_query) or die($db->error);
...
///////////////////////////////////////////////////////////////////////
    ...
    function f_getTestCountByReleaseByTemplate($template_id,$release_id)
    {
        global $db;
        $count = 0;
        $sql_query_current = "select count(*) as count from test_execution where template_id = $template_id and release_id = $release_id";
...
///////////////////////////////////////////////////////////////////////
    ...
    function f_getTestLabelListByReleaseByTemplate($release_id,$template_id)
    {
        ...
        $sql_query = "select label, id from test_execution where release_id = $release_id and template_id = $template_id and status > 1 order by id desc";
        $result = $db->query($sql_query) or die($db->error);
...
///////////////////////////////////////////////////////////////////////////////////////////
    //Return a hash of test_name => [test_id1,test_id2...]
    function f_getTestNameListByReleaseByTemplate($release_id, $template_id)
    {
        global $db;
        $test_name_list = array();
        //$sql_query_current searches through ongoing tasks
        //$sql_query_complete searches through completed tasks
        $sql_query_current = "select tc.name,te.id from test_execution as te,task_queue as tq, test_case as tc where te.task_id = tq.task_id and tq.release_id = $release_id and tq.template_id = $template_id and te.status > 1 and tc.test_id=te.test_id order by te.id desc";
        $sql_query_complete = "select tc.name,te.id from test_execution as te,completed_task as ct, test_case as tc where te.task_id = ct.task_id and ct.release_id = $release_id and ct.template_id = $template_id and tc.test_id=te.test_id order by te.id desc";
        $result_current = $db->query($sql_query_current) or die($db->error);
        $result_complete = $db->query($sql_query_complete) or die($db->error);
        ...
        if($result_current->num_rows > 0)
        {
            while ($row = $result_current->fetch_assoc())
            {
                ...
                $test_execution_id = $row['id'];
                //echo "$test_id | ";
                $test_name_list[$name][] = $test_execution_id;
            }//end of while on ongoing tasks
        }
        //Get labels and test execution ids from completed tasks
        if($result_complete->num_rows > 0)
    ...
    ///////////////////////////////////////////////////////////////////////////////////////////
    //Return a hash that contains the latest event_name, and event_time by template_id for
    //the given release_id
    function f_getLatestEventPerTemplateByRelease($release_id)
    {
        global $db;
        $events_list = array();
        $sql_query = "select template_id, event_name, max(id),event_time from (select eh.template_id, ed.event_name, eh.id,eh.event_time from event_history as eh, event_definition as ed where eh.release_id = $release_id and eh.event_id = ed.event_id) as tmp group by template_id";
        $result = $db->query($sql_query) or die($db->error);
        ...
        while($row = $result->fetch_assoc())
        {
            ...
            $events_list[$template_id] = array($event_name,$event_time);
        }//end of while
        return $events_list;
    }
    //end of f_getLatestEventPerTemplateByRelease
    ///////////////////////////////////////////////////////////////////////////////////////////
    //Return a hash that contains the latest event_name, and event_time by release_id for
    //the given template_id. Keep in mind release_id = 0 is being filtered out.
    //Those are events like template/instance suspensions, where no release applies to it.
    function f_getLatestEventPerReleaseByTemplate($template_id)
    {
        global $db;
        $events_list = array();

        //Result is order by eh's ID, so the latest event is on top, then by release_id in descending order
        //So the release with the latest event will go on top
        $sql_query = "select release_id, event_name, max(id) as id,event_time from (select eh.release_id, ed.event_name, eh.id, eh.event_time from event_history as eh, event_definition as ed where eh.template_id = $template_id and eh.event_id = ed.event_id and eh.release_id != 0) as tmp group by release_id order by id desc";
        $result = $db->query($sql_query) or die($db->error);
        //Display template only if there's event in that template for the specified release
        //So to avoid have "BAT: "
        while($row = $result->fetch_assoc())
        {
            ...
            $event_time = $row['event_time'];
            $events_list[$release_id] = array($event_name,$event_time);
        }//end of while
        return $events_list;
    }

///////////////////////////////////////////////////////////////////////////////////////////
    // //For the given template_id, return a hash of related release_id and test labels
    // //release_id => [label1,label2,label3...]
    // function f_getTestLabelListByTemplate($template_id)
    // {
    //     global $db;
    //     $list = array();
    //     $sql_query = "select release_id, distinct label from test_execution order by id desc";

    //     //All under the $template_id
    //     //Establish releases in order of get releases and its max test_excution id per release
    //     select release_id max(id) as teid from test_execution group by release_id order by teid

    //     //Under a release, get all labels and their max test execution id
    //     select label, max(id) as teid from test_excution group by label where release_id = release_id order by teid


    //     //complete catch order by id. Since everything is in desc order of id, it guarantees the release is in order
    //     //of last execution, and labels are in order of last execution.
    //     select release_id, label from test_execution where template_id = $template_id order by id desc
    // }
