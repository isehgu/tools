...
$(document).ready(function(){
  ...
  //Report user click on either report or result
  $('td').on('click','.test_report',function(){
    var rid = $(this).attr('data-rid');
    $.ajax({
        type: 'post',
        url:  'user_click.php',
        data: 'rid='+rid+'&target=view_report',
        success: function(data){
          console.log(data);
        }
    });//end of ajax
  });//end of user click report
  
  $('td').on('click','.test_result',function(){
    var rid = $(this).attr('data-rid');
    $.ajax({
        type: 'post',
        url:  'user_click.php',
        data: 'rid='+rid+'&target=view_result',
        success: function(data){
          console.log(data);
        }
    });//end of ajax
  ...
  //POC required -- auto-launching of tests after completing upgrade
  $('td').on('click','.upgrade_btn',function(){
    var env = $(this).closest('td').siblings('.upgrade_env').text();
    var app = $(this).closest('td').siblings('.upgrade_app').text();
    var version = $(this).closest('td').siblings('.upgrade_version').text();
    var btn = $(this).text();
    if(btn == 'Complete Upgrade' && app == 'Core')
    {
      if(env == 'OAT') mydata = 'suites[]=14&label='+env+' auto-test '+app+' on version '+version;
      if(env == 'PAT') mydata = 'tests[]=37&label='+env+' auto-test '+app+' on version '+version
      $.ajax({
        type: 'get',
