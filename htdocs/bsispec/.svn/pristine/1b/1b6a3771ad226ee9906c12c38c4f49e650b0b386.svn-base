...
function db_connect($type) {
	switch ($type) {
		case "iors":
			$db = mssql_connect("dd-sql01.test.ise.com\SQM1,59897", "iors_tms_gts01", "iors_tms_gts01");
			mssql_select_db("iors_tms_gts01", $db);
		break;
		case "precise":
			$db = mssql_connect("dd-sql01.test.ise.com\SQM1,59897", "bsi_tms", "bsi_tms");
			mssql_select_db("bsi_tms", $db);
		break;
		case "cfg":
			$db = mssql_connect("dd-sql01.test.ise.com\SQM1,59897", "config_mt1", "config_mt1");
			mssql_select_db("config_mt1", $db);
		break;
		case "local":
