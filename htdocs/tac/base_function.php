 ...
 */
    ...
    $approval_array = array(0=>'Pending Approval',1=>'Approved',2=>'Not Approved',3=>'Approval Not Required');
		$stat_array = array(2=>'Passed',3=>'Failed',4=>'Killed',5=>'System Error',6=>'Cancelled');
		...
		if($rownumber == 'all')
		{
			...
			$sql_query = "select tr.user_id,tr.test_id,tr.request_id,tr.label,tc.test_name,tr.status,tr.start_timestamp,tr.end_timestamp,tr.report,tr.approval_status,
                          tr.test_folder_path, tr.test_folder_name
    ...
    while($row = $result->fetch_assoc())
      ...
      $comment_count = 0;
			$report_eyeball = '';
      $result_eyeball = '';
      //$eyeball = "<span class='glyphicon glyphicon-eye-open'></span>";
			$rid = $row['request_id'];
			$label = $row['label'];
			$name = $row['test_name'];
			$status = $row['status'];
            ...
            $tfpath = $row['test_folder_path'];
            $tfname = $row['test_folder_name'];

			if($rownumber == 'all') $user = f_getUserFromId($row['user_id']);

			if($status == 2) $color_class = 'report_button_passed';
			elseif ($status == 3) $color_class = 'report_button_failed';
			elseif ($status == 5) $color_class = 'test_sys_error';
			else $color_class = 'test_killed';
      ...
      //setting eyeball
      // CBAUTISTA 09/23/2014 - Removing this because of unbareable performance
      // if(f_reportEyeball($rid)) $report_eyeball = "<span class='glyphicon glyphicon-eye-open'></span>";
      // if(f_resultEyeball($rid)) $result_eyeball = "<span class='glyphicon glyphicon-eye-open'></span>";

			echo "<tr class='test_history_row' title='$comment_count comment(s) made. Double Click to view' data-rid='$rid'>
                  <td class='test_clickable $color_class hstatus'>$stat_array[$status]</td>";

			if(!$report || $report == 'None')
			{
				echo "<td class='tap_report'>$report</td>";
			}
			else
			{
                $report = 'http://asg.ise.com/reports/' . $report;
                echo "<td><div class='tap_report'><a  data-rid='$rid' class='test_report' title='Click for TAP (Robot Framework) Report' href='$report' target='_blank'>Report $report_eyeball</a></div>";
                if($tfpath != ''){
                    //$tfpath = 'http://asg.ise.com/' . $tfpath;
                    $tfpath = "..//" . $tfpath;
                    if(file_exists($tfpath)){
                        echo "<div class='result_files'><a data-rid='$rid' class='test_result' title='Click to see any result files' href='$tfpath' target='_blank'>Results $result_eyeball</a></div></div></td>";
                    }
                    else if(file_exists($tfpath.".7z")){
                        echo "<div class='result_files'><a data-rid='$rid' class='test_result' title='Due to size constraints, please download the 7-Zip zip file to view the full test results'".
                            " href='$tfpath.7z'>Results (7z zip) $result_eyeball</a></div></div></td>";
                    }
                    else if(file_exists($tfpath.".zip")){
                        echo "<div class='result_files'><a data-rid='$rid' class='test_result' title='Due to size constraints, please download the zip file to view the full test results'".
                            " href='$tfpath.zip'>Results (zip) $result_eyeball</a></div></div></td>";
                    }
                }
                else{
                    echo "<div class='result_files' title='No Results are available'>None</div></div></td>";
                }
			}

            echo "<td class='test_clickable'>$env</td>
                  <td class='test_clickable'>$rid</td>";
            echo "<td class='test_clickable'>$label</td>";

			echo "<td class='test_clickable'>$name</td>";
			if($rownumber == 'all') echo "<td class='test_clickable'>$user</td>";
            echo "<td class='test_clickable time_display'>$stime</td>";
            echo "<td class='test_clickable time_display'>$etime</td>";
      ...
      else //Now the version exists, thus new release detected
      {
        ...
        else //if upgrade is completed, do nothing and disable btn
        {
          ...
          $upgrade_btn = 'Upgrade Completed';
			...
			echo "<tr>
							<td class='upgrade_env'>$env</td>
							<td class='upgrade_app'>$app</td>
							<td class='upgrade_version'>$version</td>
              <td>
    ...
    //TAC 1.4 -- status color
		...
		$output = "<tr class='test_history_row' title='$comment_count comment(s) made. Double Click to view' data-rid='$rid'>
			...
			else
			{
				$report = 'http://asg.ise.com/reports/' . $report;
				$output .= "
					<td><a  data-rid='$rid' class='test_report' href='$report' target='_blank'>Link to Report</a></td>
				";
			}
  ...
  function f_displayTestDetail($rid)
  {
    ...
    //User click on report
    $report_view_by = '';
    $report_sql_query = "select distinct user_id from user_action_history where action_name = 'view_report' and action_id in (select action_id from test_history_action_history where test_request_id = $rid)";
		$result = $db->query($report_sql_query) or die($db->error);
    while($row=$result->fetch_assoc())
    {
      $uid = 0;
      $uid = $row['user_id'];
      //if $report_view_by is not empty, then append ',' and another user
      //otherwise just give it a user
      if($report_view_by) $report_view_by .= ', '.f_getUserFromId($uid);
      else $report_view_by = f_getUserFromId($uid);
    }//End of while on report user click
    echo "<p><strong>Report Viewed By: </strong> $report_view_by</p>";

    //User click on results
    $result_view_by = '';
    $result_sql_query = "select distinct user_id from user_action_history where action_name = 'view_result' and action_id in (select action_id from test_history_action_history where test_request_id = $rid)";
		$result = $db->query($result_sql_query) or die($db->error);
    while($row=$result->fetch_assoc())
    {
      $uid = 0;
      $uid = $row['user_id'];
      //if $result_view_by is not empty, then append ',' and another user
      //otherwise just give it a user
      if($result_view_by) $result_view_by .= ', '.f_getUserFromId($uid);
      else $result_view_by = f_getUserFromId($uid);
    }//End of while on result user click
    echo "<p><strong>Result Viewed By: </strong> $result_view_by</p>";
  ...
  //Given test request ID, return True if report has been viewed by someone
  //return False if report has not been viewed.
  function f_reportEyeball($rid)
  {
    global $db;
    $sql_query = "SELECT COUNT( * ) AS count FROM  `user_action_history` WHERE action_name =  'view_report' AND user_id not in (2,3) AND action_id IN (SELECT action_id FROM test_history_action_history WHERE test_request_id = $rid)";
    $result = $db->query($sql_query) or die($db->error);
    $row=$result->fetch_assoc();
  ...
  //Given test request ID, return True if result has been viewed by someone
  //return False if result has not been viewed.
  function f_resultEyeball($rid)
  {
    global $db;
    $sql_query = "SELECT COUNT( * ) AS count FROM  `user_action_history` WHERE action_name =  'view_result' AND user_id not in (2,3) AND action_id IN (SELECT action_id FROM test_history_action_history WHERE test_request_id = $rid)";
    $result = $db->query($sql_query) or die($db->error);
    $row=$result->fetch_assoc();
    if($row['count'] > 0) return true;
    else return false;
