    ...
    while($row = $result->fetch_assoc())
      ...
      $comment_count = f_getCommentCount($rid);
			...
			else
			{
                ...
                echo "<td><div><div class='tap_report'><a title='Click for TAP (Robot Framework) Report' href='$report' target='_blank'>Report</a></div>";
                if($tfpath != ''){
                    $tfpath = 'http://asg.ise.com/' . $tfpath;
                    echo "<div class='result_files'><a title='Click to see any result files' href='$tfpath' target='_blank'>Results</a></div></div></td>";
                }
                else{
